...
"""
from .NewsSentimentScanner.sentiment_analysis import run_sentiment_module as _run_sentiment_scanner


def run_sentiment_module(ticker: str, num_articles_per_query: int = 10) -> dict:
    """
    Run news sentiment analysis for a ticker and return fusion-compatible result.

    Args:
        ticker: Stock symbol (e.g. "AAPL", "RELIANCE.NS").
        num_articles_per_query: Articles to fetch per query (default 10).
    ...
    """
    try:
        result = _run_sentiment_scanner(ticker.strip(), num_articles_per_query)
    except Exception:
